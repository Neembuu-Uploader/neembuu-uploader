 ...
 * @author Paralytic
 */
public class SharedSxAccount extends AbstractAccount{
    ...
    public SharedSxAccount() {
        KEY_USERNAME = "shrdsxusername";
        KEY_PASSWORD = "shrdsxpassword";
        HOSTNAME = "Shared.sx";
    }
    ...
    public void login() {
        ...
        try {
            ...
            NULogger.getLogger().info("Trying to log in to Shared.sx");
            httpPost = new NUHttpPost("http://shared.sx/request");

            List<NameValuePair> formparams = new ArrayList<NameValuePair>();
            formparams.add(new BasicNameValuePair("action", "login"));
            formparams.add(new BasicNameValuePair("email", getUsername()));
            formparams.add(new BasicNameValuePair("password", MD5(getPassword())));
            ...
            // {"status":true,"code":null}

            if (!CookieUtils.getCookieValue(httpContext, "user").isEmpty() && responseString.contains("true")) {
                EntityUtils.consume(httpResponse.getEntity());
                loginsuccessful = true;
                username = getUsername();
                password = getPassword();
                hostsAccountUI().hostUI(HOSTNAME).setEnabled(true);
                NULogger.getLogger().info("Shared.sx login successful!");
