...
import neembuu.uploader.accounts.LineStorageAccount;
 ...
 */
@SmallModule(
    exports={LineStorage.class,LineStorageAccount.class},
    interfaces={Uploader.class,Account.class},
    name="LineStorage.com"
)
public class LineStorage extends AbstractUploader{
    
    LineStorageAccount lineStorageAccount = (LineStorageAccount) getAccountsProvider().getAccount("LineStorage.com");
    ...
    public LineStorage() {
        ...
        if (lineStorageAccount.loginsuccessful) {
            host = lineStorageAccount.username + " | LineStorage.com";
        }
        maxFileSizeLimit = 104857600L; // 100 MB (default)
        
    }

    private void initialize() throws Exception {
        responseString = NUHttpClientUtils.getData("http://linestorage.com/", httpContext);
    ...
    public void run() {
        try {
            if (lineStorageAccount.loginsuccessful) {
                if (lineStorageAccount.isPremium()) {
                    httpContext = lineStorageAccount.getHttpContext();
                    userType = "prem";
                    maxFileSizeLimit = Long.MAX_VALUE; // Unlimited
                    NULogger.getLogger().info("** LineStorage.com ** => Proceeding as a premium user.");
                } else {
                    userType = "reg";
                    httpContext = lineStorageAccount.getHttpContext();
                    maxFileSizeLimit = 1073741824L; // 1 GB
                    NULogger.getLogger().info("** LineStorage.com ** => Proceeding as a regular user.");
                }
            } else {
                userType = "anon";
            ...
            NULogger.getLogger().info("Now uploading your file into LineStorage.com");
            uploading();
            httpResponse = httpclient.execute(httpPost, httpContext);
            responseString = EntityUtils.toString(httpResponse.getEntity());
            FileUtils.saveInFile("LineStoragePostResponse.html", responseString);
            ...
            if (upload_fn != null) {
                httpPost = new NUHttpPost("http://linestorage.com/");
                ...
                httpResponse = httpclient.execute(httpPost, httpContext);
                responseString = EntityUtils.toString(httpResponse.getEntity());
                FileUtils.saveInFile("LineStorageGetLinksResponse.html", responseString);
