...
import neembuu.uploader.accounts.ExLoadAccount;
 ...
 */
@SmallModule(
    exports={ExLoad.class,ExLoadAccount.class},
    interfaces={Uploader.class,Account.class},
    name="Ex-Load.com"
)
public class ExLoad extends AbstractUploader implements UploaderAccountNecessary{
    
    ExLoadAccount exLoadAccount = (ExLoadAccount) getAccountsProvider().getAccount("Ex-Load.com");
    ...
    public ExLoad() {
        ...
        host = "Ex-Load.com";
        if (exLoadAccount.loginsuccessful) {
            host = exLoadAccount.username + " | Ex-Load.com";
        }
        maxFileSizeLimit = 75161927680L; // 71,680 MB = 70 GB (default)
        
    }

    private void initialize() throws Exception {
        responseString = NUHttpClientUtils.getData("http://ex-load.com/exupload", httpContext);
    ...
    public void run() {
        try {
            if (exLoadAccount.loginsuccessful) {
                httpContext = exLoadAccount.getHttpContext();
                maxFileSizeLimit = 75161927680L; // 71,680 MB = 70 GB
            } else {
                host = "Ex-Load.com";
                uploadInvalid();
                return;
            ...
            // http://167.114.117.70/tmp
            // http://167.114.117.70/cgi-bin/upload.cgi?upload_id=750808075715&js_on=1&utype=reg&upload_type=file
            ...
            NULogger.getLogger().info("Now uploading your file into Ex-Load.com");
            uploading();
